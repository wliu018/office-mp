  ...
  () => {
    ...
    const setTokenInfo = (val: IAuthLoginRes) => {
      ...
      if (isSingleTokenRes(val)) {
        ...
        uni.setStorageSync('accessTokenExpireTime_', expireTime)
      }
      else if (isDoubleTokenRes(val)) {
        // 双token模式
        const accessExpireTime = now + val.accessExpiresIn * 1000
        const refreshExpireTime = now + val.refreshExpiresIn * 1000
        uni.setStorageSync('accessTokenExpireTime_', accessExpireTime)
    ...
    const isTokenExpired = computed(() => {
      ...
      const expireTime = uni.getStorageSync('accessTokenExpireTime_')
    ...
    const logout = async () => {
      try {
        ...
        // 清除存储的过期时间
        uni.removeStorageSync('accessTokenExpireTime_')
